  ...
  constructor(props) {
    ...
    componentWillMount() {
        AsyncStorage.getItem('userID').then((value)=>{
            ...
            console.log("Logged in user: " + this.state.userID);
            url = 'http://10.9.5.153:8080/NetwerkBackend/NetwerkBackend?functionToExecute=getAllUsers&userID=' + this.state.userID;
            fetch(url, {
              method: 'GET',
              headers: {
   ...
   handleYup (card) {
      ...
      url = 'http://10.9.5.153:8080/NetwerkBackend/NetwerkBackend?functionToExecute=addToLiked&likerID=' + this.state.userID + '&likedID=' + card.userID;
      fetch(url, {
          method: 'GET',
          headers: {
              'Content-Type': 'application/json',
      ...
      .then((responseJson) => {
         this.setState({
            ...
            if (responseJson == true) {
              Alert.alert("You and " + card.displayName + " have been matched!");
            } else {
